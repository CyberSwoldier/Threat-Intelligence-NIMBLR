...
# CONFIGURATION
# -------------------------------
st.set_page_config(page_title="Weekly Threat Intelligence", layout="wide", initial_sidebar_state="expanded")
...
if page == "About":
    ...
    # COUNTRY FILTER
    # -------------------------------
    all_countries = sorted(pd.Series(pd.concat([selected_report[col] for col in country_columns], ignore_index=True))
                           .dropna().unique())
    all_countries = [c for c in all_countries if c != "None"]
    selected_country = st.selectbox("Filter by country", ["All"] + all_countries)
    if selected_country != "All":
        selected_report = selected_report[selected_report[country_columns].apply(
            lambda row: selected_country in row.values, axis=1)]

    # -------------------------------
    # MULTI-COUNTRY COMPARISON
    # -------------------------------
    selected_countries = st.multiselect("Compare countries (up to 5)", all_countries, default=all_countries[:3])
    compare_report = None
    if selected_countries:
        compare_report = selected_report[selected_report[country_columns].apply(
            lambda row: any(c in row.values for c in selected_countries), axis=1)]
    ...
    if ttp_columns:
        ...
        st.metric("MITRE TTPs", len(set(all_ttps_flat)), delta=None, help="Number of unique MITRE TTPs")
    else:
        st.metric("MITRE TTPs", 0)
    ...
    if country_columns:
        ...
        if not all_countries_series.empty:
            ...
            fig_globe.update_geos(projection_type="orthographic", showcoastlines=True,
                                  coastlinecolor="lightblue", showland=True, landcolor="#0E1117",
                                  showocean=True, oceancolor="#0E1117", showframe=False, bgcolor="#0E1117")
            fig_globe.update_layout(title=f"Countries affected in {selected_date}", paper_bgcolor="#0E1117",
                                    plot_bgcolor="#0E1117", font=dict(color="white"), margin=dict(l=0,r=0,t=40,b=0),
                                    height=700)
    ...
    if search_term:
        ...
        if not filtered_items.empty:
            ...
            output.seek(0)
            st.download_button("📥 Download Filtered Results", data=output,
