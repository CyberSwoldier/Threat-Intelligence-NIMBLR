...
if st.sidebar.button("Fetch Latest News"):
    ...
    st.sidebar.write("Click the button to fetch latest threat intel news.")
...
def plot_heatmap(df, x_col, y_col, title):
    # Create full matrix to fill empty cells
    ...
    hm_filled = hm.fillna(0)

    # Colors: black for zero, smooth yellow gradient for positive counts
    colorscale = [[0, "black"]] + [
